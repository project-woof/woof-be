...
export default {
  async fetch(
    ...
    try {
      ...
      return new Response("Booking Service: Route not found", { status: 404, headers: corsHeaders });
...
async function getBookingsByUserId(env: Env, headers: HeadersInit, userId: string, limit: number, offset: number): Promise<Response> {
...
// Get booking by id
async function getBookingById(env: Env, headers: HeadersInit, bookingId: string): Promise<Response> {
  const { results } = await env.PETSITTER_DB.prepare(`
...
async function createBooking(env: Env, headers: HeadersInit, request: Request,): Promise<Response> {
...
// Delete a booking
async function deleteBooking(env: Env, headers: HeadersInit, bookingId: string): Promise<Response> {
